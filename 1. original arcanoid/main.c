...
void autoMoveBall(){
    ...
    if((mas[ball.iy][ball.ix]=='#') || (mas[ball.iy][ball.ix]=='@')){  //СТОЛКНОВЕНИЯ

        if(mas[ball.iy][ball.ix]=='@') hitCnt++;           //подсчет кол-во отскоков от ракетки
...
int main()
{
    ...
    do{
        ...
        if (ball.iy>height)   //уход шарика вниз за экран
        {
            run=false;
            if (hitCnt>maxHitCnt)   //макс. кол-во ударов
                maxHitCnt=hitCnt;
            hitCnt=0;
        }
